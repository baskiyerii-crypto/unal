...
export function FloatingContact({
  ...
  whatsappMessage: customMsg,
}: FloatingContactProps) {
  const [mounted, setMounted] = useState(false);

  useEffect(() => {
    // Smooth fade-in delay after page loads
  ...
  const defaultMsg = 'Merhaba MenakYapı, çatı/kenet çatı hizmetleriniz hakkında ücretsiz teklif ve bilgi almak istiyorum.';
  const whatsappMessage = encodeURIComponent(customMsg || defaultMsg);
  ...
  return (
    <div className="fixed bottom-5 left-0 right-0 z-50 pointer-events-none px-4 sm:px-6 max-w-7xl mx-auto flex items-center justify-between">
      ...
      <div className="relative pointer-events-auto group">
        ...
        {/* Expanded Badge / Tooltip on Desktop */}
        <div className="absolute left-full ml-3 top-1/2 -translate-y-1/2 hidden md:flex items-center gap-2 bg-[#24292E] text-white text-xs font-semibold px-3 py-1.5 rounded-full shadow-lg border border-[#D4AF37]/30 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none">
          ...
          <span>Hemen Usta ile Görüş: <strong className="text-[#D4AF37]">{formattedPhone}</strong></span>
        </div>

        <a
          href={`tel:${phone}`}
          aria-label="Telefon İle Ulaşın"
          className="relative flex items-center justify-center w-14 h-14 rounded-full bg-linear-to-br from-[#24292E] to-[#111827] text-[#D4AF37] border-2 border-[#D4AF37]/50 shadow-xl shadow-black/20 hover:scale-110 hover:border-[#D4AF37] transition-all duration-300 group"
        >
      ...
      <div className="relative pointer-events-auto group">
        ...
        <a
          href={`https://wa.me/90${whatsappClean}?text=${whatsappMessage}`}
          target="_blank"
          rel="noopener noreferrer"
          aria-label="WhatsApp İle Teklif Alın"
          className="relative flex items-center justify-center w-14 h-14 rounded-full bg-linear-to-br from-[#25D366] to-[#128C7E] text-white shadow-xl shadow-emerald-600/30 hover:scale-110 hover:shadow-emerald-600/50 transition-all duration-300"
        >
